...
# -f: verifica se il file esiste ed è un file regolare
if [ ! -f "$FILE_LOG" ]; then
...
# ${VAR}: espansione della variabile (più sicuro con le graffe)
...
awk -v id="$ID_CLIENTE" -v di="$DATA_INIZIO_FMT" -v df="$DATA_FINE_FMT" '
    BEGIN { FS="|"; OFS="|" }
    {
        ...
        # /^[ \t]+/: regex - inizio riga seguito da spazi o tab (uno o più)
        ...
        # Verifica se l\'ID è presente nel percorso (campo 5)
        if (index($5, id) > 0) {
            # Verifica se la data è nel range specificato
            # Confronto stringhe funziona perché YYYYMMDD è ordinabile
